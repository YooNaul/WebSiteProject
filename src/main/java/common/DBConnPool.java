...
DBCP(DataBase Connection Pool : 커넥션풀)
	: DB와 견결된 커넥션 객체를 미리 만들어 풀(Pool)에 저장해 뒀다가
	필요할떄 가져다쓰고 반납하는 기법을 말한다. DB에 부하를 줄이고
	자원을 효율적으로 관리할 수 있다. 게임에서는 풀림시스템(Pooling System)
	이라는 용어로 사용한다.
 */
public class DBConnPool {
    ...
    public DBConnPool() {
        try {
            ...
            여기서 얻어온느 톰켓의 루트 디렉토리명은 java:comp/env로 이미 정해져
            있으므로 그대로 사용하면 된다.
             */
